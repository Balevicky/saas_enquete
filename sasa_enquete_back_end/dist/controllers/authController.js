...
class AuthController {
    ...
    static async signupTenant(req, res) {
        try {
            // const id = await generateId();
            // const token = await generateId(32);
            const { tenantName, tenantSlug, ownerEmail, ownerPassword, ownerFullName, } = req.body;
    ...
    static async forgotPassword(req, res) {
        ...
        <p>You requested a password reset for <strong>${userTenant.tenant.name}</strong>.</p>
        <p>
          <a href="${resetLink}">
            Reset my password
          </a>
